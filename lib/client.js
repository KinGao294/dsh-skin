...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			let skinBound;
			// Guard against the applyWallpaper -> overrideTokens -> theme/change
			// -> syncSkin -> applyWallpaper loop: re-shade only when the active
			// skin actually changed (fixes RangeError: Maximum call stack size
			// exceeded when a wallpaper is set; see issue #1).
			let lastShadedSkin = null;
			const syncSkin = (snapshot) => {
				...
				if (readWallpaper() !== null && snapshot.active?.id !== lastShadedSkin) {
					lastShadedSkin = snapshot.active?.id ?? null;
					applyWallpaper(ctx);
				}
			};
