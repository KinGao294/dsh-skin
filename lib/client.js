...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * Boot grace window (ms) during which theme/change fallbacks onto a
		 * built-in preference re-assert the saved skin. The official
		 * ThemeRuntime re-adopts its host-persisted appearance (system /
		 * light / dark) asynchronously when its settings scope lands, and that
		 * adoption can fire after the fixed 0/80ms reasserts; the window keeps
		 * the saved skin alive until the scope has settled.
		 */
		const BOOT_REASSERT_GRACE_MS = 4000;
		...
		function apply(ctx) {
			...
			// Restore the saved skin across boot. The official ThemeRuntime
			// re-adopts its host-persisted appearance (system/light/dark)
			// asynchronously when the settings scope lands; on rc.6 that
			// adoption routinely fires after the fixed 0/80ms reasserts below
			// and clobbers a third-party skin back to the built-in preference,
			// so a restart visibly drops the saved skin. Re-assert on the next
			// ticks AND on every theme/change that falls back onto a built-in
			// preference during the boot grace window. The window is time-boxed
			// (not "until first win") because the settings scope can refresh
			// more than once around boot (connection reset, forwarded document
			// updates); picking 默认/Default clears the stored skin, which ends
			...
			const stopBootReassert = ctx.on("theme/change", (snapshot) => {
				if (snapshot.preference === DEFAULT_SKIN || snapshot.preference === "light" || snapshot.preference === "dark") reassertSavedSkin();
			});
			const bootReassertTimer = setTimeout(stopBootReassert, BOOT_REASSERT_GRACE_MS);
			ctx.effect(() => () => {
				clearTimeout(bootReassertTimer);
			}, "dsh-skin: boot reassert cleanup");
