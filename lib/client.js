...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Wallpaper display mode (cover/contain/stretch/tile; default cover). */
		...
		/** Re-entrancy guard: overrideTokens() emits theme/change, which would otherwise re-enter applyWallpaper forever. */
		...
		function shadeTokens(ctx) {
			// overrideTokens() emits theme/change synchronously; syncSkin() calls
			// applyWallpaper() on every theme/change, so without this guard loading a
			// wallpaper would re-enter here forever and freeze the UI.
		...
		function applyWallpaper(ctx) {
			...
			// Avoid rewriting the same data-URL/background on every slider tick;
			// that forces the browser to re-parse and re-decode a multi-hundred-KB string.
			if (wallpaperEl.style.backgroundImage !== nextBackground) wallpaperEl.style.backgroundImage = nextBackground;
			if (wallpaperEl.style.filter !== nextFilter) wallpaperEl.style.filter = nextFilter;
			if (wallpaperEl.style.backgroundSize !== nextSize) wallpaperEl.style.backgroundSize = nextSize;
			if (wallpaperEl.style.backgroundRepeat !== nextRepeat) wallpaperEl.style.backgroundRepeat = nextRepeat;
			if (wallpaperEl.style.backgroundPosition !== nextPosition) wallpaperEl.style.backgroundPosition = nextPosition;
			shadeTokens(ctx);
		}

		/** Pending requestAnimationFrame id for coalesced wallpaper application. */
		let wallpaperApplyRaf = null;

		/**
		 * Coalesce wallpaper re-application to one frame. Dragging a slider fires
		 * many input events; applying theme overrides on every one makes the UI
		 * stutter. The store still updates immediately so the slider stays live.
		 */
		function scheduleWallpaperApply(ctx) {
			if (wallpaperApplyRaf !== null) return;
			wallpaperApplyRaf = requestAnimationFrame(() => {
				wallpaperApplyRaf = null;
				applyWallpaper(ctx);
			});
		}

		/** Cancel a pending wallpaper apply (used on teardown). */
		...
		/** Wallpaper row store: url + opacity + blur + fit, written only by this plugin. */
		function createWallpaperStore() {
			return (0, _runtime_client.defineStore)({
				...
				actions: {
					sync: (d, url, opacity, blur, fit, revision) => {
						if (revision <= d.revision) return;
						d.url = url;
						d.opacity = opacity;
						d.blur = blur;
		 ...
		 */
		function WallpaperRow({ t, setWallpaper, setOpacity, setBlur, setFit, useStore }) {
				...
				children: [
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							(0, react_jsx_runtime.jsxs)("div", {
								style: styles.fitRow,
								children: WALLPAPER_FITS.map((value) => (0, react_jsx_runtime.jsx)("button", {
									type: "button",
		...
		function apply(ctx) {
			...
			const syncWallpaper = () => {
				wallpaperRevision += 1;
				wallpaperBound?.sync(readWallpaper(), readWallpaperOpacity(), readWallpaperBlur(), readWallpaperFit(), wallpaperRevision);
			};
			applyWallpaper(ctx);
			syncWallpaper();
			ctx.effect(() => () => {
			...
			let skinBound;
			// DSH's ThemeRuntime persists only built-in preferences (system/light/
			// dark) through the host settings scope. A third-party skin id lives in
			// localStorage, but on startup the host scope can adopt its saved
			// preference (often "system" or "dark") and reset our in-memory skin.
			// Keep re-asserting the saved skin until the user explicitly picks
			// "default" (or another skin) through this plugin.
			let skinUserCleared = false;
			const reassertSavedSkin = () => {
				if (skinUserCleared) return;
				const saved = readSavedSkin();
			...
			// Also re-assert after the current tick, in case the host settings
			// scope adopted before this listener was registered.
			setTimeout(reassertSavedSkin, 0);
			...
			const skinInjected = (actions) => {
				...
				return {
					setSkin: (id) => {
						skinUserCleared = id === DEFAULT_SKIN;
			...
			const wallpaperInjected = (actions) => {
				...
				return {
					setWallpaper: (url) => {
						...
						// Update the row immediately; coalesce the expensive theme
						// re-override (and background decode) to the next frame.
						syncWallpaper();
					...
					setFit: (fit) => {
						...
						writeStorage(WALLPAPER_FIT_KEY, value);
						syncWallpaper();
						scheduleWallpaperApply(ctx);
					}
				};
