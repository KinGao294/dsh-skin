...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Base URL of the local media server used for video uploads. Point it at your own server (e.g. a static host with a POST /upload?name= endpoint returning {"url": ...}) or set to "" to disable the upload button (the URL input below still works). */
		const MEDIA_SERVER = "http://127.0.0.1:8099";
		/** localStorage key holding the default-wallpaper quality tier (auto/720p/1080p/original). */
		const WALLPAPER_QUALITY_KEY = "dsh-skin:wallpaper-quality";
		/** Default wallpaper video per quality tier. Point each entry at your own media
		 *  server (or a data URL); leave all empty to disable the default wallpaper
		 *  entirely (behaviour falls back to no wallpaper until the user sets one). */
		const WALLPAPER_TIERS = {
			"720p": "",
			"1080p": "",
			original: ""
		};
		...
		const zh = {
			...
			"background.choose": "选择图片/视频",
			"background.quality": "背景质量",
			"background.qualityAuto": "自动(按屏幕)",
			"background.quality720p": "720p (省电)",
			"background.quality1080p": "1080p (清晰)",
			"background.qualityOriginal": "原始 4K",
			"background.uploadFail": "视频上传失败：请检查媒体服务地址（MEDIA_SERVER）或改用下方 URL 输入",
			"background.urlPlaceholder": "或粘贴视频/图片 URL",
		...
		const en = {
			...
			"background.choose": "Choose image/video",
			"background.quality": "Wallpaper quality",
			"background.qualityAuto": "Auto (by screen)",
			"background.quality720p": "720p (battery)",
			"background.quality1080p": "1080p (sharp)",
			"background.qualityOriginal": "Original 4K",
			"background.uploadFail": "Video upload failed: check the media server (MEDIA_SERVER) or paste a URL below",
			"background.urlPlaceholder": "Or paste a video/image URL",
		...
		/** Selected default-wallpaper quality tier: auto/720p/1080p/original (default auto). */
		function readQuality() {
			const value = readStorage(WALLPAPER_QUALITY_KEY);
			return value === "720p" || value === "1080p" || value === "original" ? value : "auto";
		}

		/** Pick the tier matching this screen: ≤1080p → 720p, 2K/4K → 1080p. */
		function pickAutoQuality() {
			const width = window.screen?.width || 1920;
			const height = window.screen?.height || 1080;
			const dpr = window.devicePixelRatio || 1;
			const maxPhysical = Math.max(width, height) * dpr;
			return maxPhysical > 2300 ? "1080p" : "720p";
		}

		/** Default wallpaper URL for the current tier; null when tiers are unconfigured. */
		function defaultWallpaperUrl() {
			const tier = readQuality() === "auto" ? pickAutoQuality() : readQuality();
			return WALLPAPER_TIERS[tier] || WALLPAPER_TIERS["720p"] || null;
		}

		/** Wallpaper data URL; falls back to the tiered default when unset. */
		function readWallpaper() {
			const value = readStorage(WALLPAPER_KEY);
			return value !== null && value.length > 0 ? value : defaultWallpaperUrl();
		}
		...
		/** True when the wallpaper URL is a playable video (mp4/webm/ogv/mov/m3u8). */
		function isVideoUrl(url) {
			return /^(data:video\/|blob:)/i.test(url) || /\.(mp4|webm|ogv|mov|m3u8)(\?|#|$)/i.test(url);
		}

		/** Upload a video to the configured media server; resolves to its URL or null on failure. */
		function uploadVideo(file) {
			if (MEDIA_SERVER === "") return Promise.resolve(null);
			return fetch(`${MEDIA_SERVER}/upload?name=${encodeURIComponent(file.name)}`, {
				method: "POST",
		...
		/** Re-entrancy guard: shadeTokens -> overrideTokens emits theme/change synchronously, which re-enters syncSkin -> applyWallpaper; the guard breaks the infinite loop. */
		let wallpaperApplying = false;

		/** Apply (or clear) the wallpaper layer and its token shading. */
		function applyWallpaper(ctx) {
			if (wallpaperApplying) return;
			...
			try {
				...
				if (wallpaperEl === null || !document.body.contains(wallpaperEl) || (video && wallpaperEl.tagName !== "VIDEO") || (!video && wallpaperEl.tagName === "VIDEO")) {
					if (wallpaperEl && wallpaperEl.tagName === "VIDEO") wallpaperEl.pause();
					wallpaperEl?.remove();
					if (video) {
		...
		function WallpaperRow({ t, setWallpaper, setOpacity, setBlur, useStore }) {
			...
			const [quality, setQuality] = (0, _react.useState)(readQuality());
			const onQualityChange = (next) => {
				setQuality(next);
				writeStorage(WALLPAPER_QUALITY_KEY, next === "auto" ? null : next);
				const url = defaultWallpaperUrl();
				if (url !== null) setWallpaper(url);
			};
			const onFile = (event) => {
				const file = event.target.files?.[0];
				if (file === void 0) return;
				event.target.value = "";
				if (/\.(mp4|webm|ogv|mov)$/i.test(file.name) || file.type.startsWith("video/")) {
					uploadVideo(file).then((url) => {
						if (url !== null) setWallpaper(url);
						else window.alert(t("background.uploadFail"));
					});
				} else {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							url !== null ? (isVideoUrl(url) ? (0, react_jsx_runtime.jsx)("video", {
								src: url,
								muted: true,
								loop: true,
								autoPlay: true,
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							(0, react_jsx_runtime.jsx)("select", {
								...
								children: [
									(0, react_jsx_runtime.jsx)("option", { value: "auto", children: t("background.qualityAuto") }),
									(0, react_jsx_runtime.jsx)("option", { value: "720p", children: t("background.quality720p") }),
									(0, react_jsx_runtime.jsx)("option", { value: "1080p", children: t("background.quality1080p") }),
									(0, react_jsx_runtime.jsx)("option", { value: "original", children: t("background.qualityOriginal") })
								]
							})
						]
					}),
