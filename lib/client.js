...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"background.urlInvalid": "请使用 http(s)、data 或 blob 地址",
			"background.videoHint": "本地视频请改用网址（不会上传到本机服务）",
			"background.hint": "图片或视频显示在主内容区与侧边栏的半透明底上，消息等内层表面保持不透明以保证可读性"
		};
		...
		const en = {
			...
			"background.urlInvalid": "Use an http(s), data, or blob URL",
			"background.videoHint": "For local video, paste a URL (nothing is uploaded)",
			"background.hint": "The image or video shows through the translucent main canvas and sidebar; inner surfaces stay opaque for readability"
		};
		//#endregion
		...
		/** Allowlisted wallpaper URL (http(s) / data:image|video / blob). */
		function sanitizeWallpaperUrl(raw) {
			if (typeof raw !== "string") return null;
			const value = raw.trim();
			if (value === "" || /["'\n\r]/.test(value)) return null;
			if (/^(https?:|data:image\/|data:video\/|blob:)/i.test(value)) return value;
			return null;
		}

		function isVideoUrl(url) {
			return /^(data:video\/|blob:)/i.test(url) || /\.(mp4|webm|ogv|mov)(\?|#|$)/i.test(url);
		}
		//#endregion
		...
		function WallpaperRow({ t, setWallpaper, setOpacity, setBlur, setFit, useStore }) {
			...
			const onFile = (event) => {
				...
				if ((file.type && file.type.startsWith("video/")) || /\.(mp4|webm|ogv|mov)$/i.test(file.name)) {
					window.alert(t("background.videoHint"));
					return;
				}
			...
			const applyUrl = () => {
				...
				if (sanitized === null) {
					window.alert(t("background.urlInvalid"));
					return;
				}
